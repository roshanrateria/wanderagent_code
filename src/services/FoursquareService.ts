...
export class FoursquareService {
  ...
  private parseServerResponse(serverPlaces: any[]): Place[] {
    if (!Array.isArray(serverPlaces)) {
      return [];
    }

    return serverPlaces.map((place: any) => ({
      id: place.fsq_place_id || place.id || '',
      name: place.name || '',
      category: place.categories?.[0]?.name || 'Unknown',
  ...
  private parseServerPlace(serverPlace: any): Place | null {
    if (!serverPlace || !serverPlace.fsq_place_id) {
      return null;
    }

    return {
      id: serverPlace.fsq_place_id,
      name: serverPlace.name || '',
      category: serverPlace.categories?.[0]?.name || 'Unknown',
      address: serverPlace.location?.formatted_address || serverPlace.location?.address || '',
      latitude: serverPlace.latitude || 0,
      longitude: serverPlace.longitude || 0,
      rating: serverPlace.rating,
      price: serverPlace.price,
      photos: serverPlace.photos?.map((photo: any) => `${photo.prefix}300x300${photo.suffix}`) || [],
      hours: serverPlace.hours?.display,
      phone: serverPlace.tel,
