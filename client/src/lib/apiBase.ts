export async function apiRequest(method: string, path: string, body?: any, sessionId?: string): Promise<Response> {
    ...
    window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
      try {
        if (typeof input === 'string' || input instanceof URL) {
          ...
          if (abs !== req.url) {
            const headers = ensureHeadersWithSession(req.headers as any);
