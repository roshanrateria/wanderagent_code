...
export default function Home() {
  ...
  const ensureSessionAndLocation = async (): Promise<string | null> => {
    ...
    // If we don't have a usable location, try to get it and persist
    const hasLoc = userLocation && isFinite(userLocation.lat) && isFinite(userLocation.lng) && (userLocation.lat !== 0 || userLocation.lng !== 0);
    if (!hasLoc) {
      try {
        ...
        if (coords) {
          ...
          try {
            await routeApiCall('POST', '/api/location', coords, sid);
          } catch {}
        }
      } catch (e: any) {
  ...
  const fetchQuick = async (type: 'coffee' | 'atm' | 'restroom') => {
    ...
    try {
      ...
      // attach best-effort browser location in headers for first-time users
      const headers: Record<string, string> = { ...(sid ? { 'session-id': sid } : (sessionId ? { 'session-id': sessionId } : {})) };
      if (userLocation && isFinite(userLocation.lat) && isFinite(userLocation.lng) && (userLocation.lat !== 0 || userLocation.lng !== 0)) {
        headers['x-user-lat'] = String(userLocation.lat);
        headers['x-user-lng'] = String(userLocation.lng);
      }
      const qs = userLocation ? `?lat=${encodeURIComponent(String(userLocation.lat))}&lng=${encodeURIComponent(String(userLocation.lng))}` : '';
      const url = `/api/quick/${type}${qs}`;
      console.log('[Quick Debug] Calling API:', url);
      console.log('[Quick Debug] Headers:', headers);
      
      const res = await routeApiCall('GET', url, undefined, sid || sessionId);
      ...
      setQuickResults(Array.isArray(data?.results) ? data.results : []);
    } catch (e: any) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-3">
              <img src="/icon.png" alt="WanderAgent Logo" className="h-8 w-8 rounded" />
                ...
                <div className="flex items-center gap-2">
                  <Button
                    ...
                    onClick={() => {
                      ...
                      // fetch('/api/itinerary', { ... }) -> use router for standalone
                      routeApiCall('PUT', '/api/itinerary', { multiDay: { days: plannedItinerary.days } }, sessionId)
                        .then(() => toast({ title: 'Saved', description: 'Multi-day plan saved' }))
                        .catch(() => {});
                    }}
                  >
