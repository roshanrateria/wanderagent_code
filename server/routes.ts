...
import { TravelPlanningAgent, MultiAgentOrchestrator } from "./services/langchain-agent";
...
export async function registerRoutes(app: Express): Promise<Server> {
  // Serve PWA manifest and service-worker for web installability (only if present)
  try {
    const path = await import('path');
    const fs = await import('fs');
    const manifestPath = path.resolve(import.meta.dirname, '..', 'client', 'public', 'manifest.webmanifest');
    const swPath = path.resolve(import.meta.dirname, '..', 'client', 'public', 'service-worker.js');

    if (fs.existsSync(manifestPath)) {
      app.get('/manifest.webmanifest', (_req, res) => res.sendFile(manifestPath));
    }
    if (fs.existsSync(swPath)) {
      app.get('/service-worker.js', (_req, res) => res.sendFile(swPath));
    }
  } catch (e) {
    // non-fatal
  ...
  // Handle deep-links from navigator.registerProtocolHandler landing page
  app.get('/open', (req, res) => {
    const url = String(req.query.url || '');
    // Redirect into SPA with deep query so client can handle
  ...
  // Generate itinerary using Multi-Agent AI system
  app.post("/api/generate-itinerary", async (req, res) => {
    try {
      ...
      if (isMultiDay) {
        // Multi-day flow with agent coordination
        ...
        // Enrich places with Wikipedia images if missing
        ...
        const coordinates: Array<[number, number]> = [
          ...
          ...enrichedPlaces.map((p: any) => [p.longitude as number, p.latitude as number] as [number, number])
        ];
        ...
        const itineraryForStorage = enrichedPlaces.map((place: any, index: number) => {
      ...
      // Single day with Multi-Agent Orchestrator
      console.log('🚀 Using Multi-Agent System for comprehensive planning...');
      ...
      const allCoords: Array<[number, number]> = [
        [preferences.location.lng, preferences.location.lat],
        ...result.data.itinerary.map((place: any) => [place.longitude as number, place.latitude as number] as [number, number])
      ];
      ...
      const routeGeometry = routeResult.decodedGeometry || [];
      const osrmInstructions = routeResult.instructionsByLeg || [];
      const optimizedRoutePath = (result.data.optimizedRoute || []).map((idx: number) => {
      ...
      const itinerary = await storage.createItinerary({
        ...
        optimizedRoute: optimizedRoutePath,
        routeGeometry,
        // @ts-ignore add osrmInstructions and budgetInsights fields
        osrmInstructions,
  ...
  app.get("/api/foursquare-place/:placeId", async (req, res) => {
    try {
  ...
  // Simple Gemini text prompt endpoint
  ...
  // ============================================
  // BUDGET TRACKING AGENT ENDPOINTS
  // ============================================

  // Analyze spending and get insights
  ...
  app.post('/api/budget/recommendations', async (req, res) => {
    try {
  ...
  app.post('/api/budget/predict', async (req, res) => {
    try {
  ...
  // Generate complete album with all photos
  app.post('/api/photos/album', async (req, res) => {
    try {
  ...
  // ============================================
  // WIKIPEDIA IMAGE FALLBACK ENDPOINT
  // ============================================

  // Get Wikipedia image for a place
  app.get('/api/wikipedia/image', async (req, res) => {
    try {
      const { lat, lon, name, radius } = req.query;

      const { wikipediaImageService } = await import('./services/wikipedia-images');

      let imageResult = null;

      // Try by coordinates if provided
      if (lat && lon) {
        ...
        if (!isNaN(latitude) && !isNaN(longitude)) {
          imageResult = await wikipediaImageService.getImageByCoordinates(
            latitude,
            longitude,
            searchRadius
          );
      ...
      // Try by name if no result from coordinates
      if (!imageResult && name) {
        imageResult = await wikipediaImageService.getImageByPlaceName(name as string);
      }
