...
Required JSON schema:
{
  ...
  "summary": "overall summary"
    ...
    const response = await client.chat.completions.create({ model, messages, ...config, stream: true });
    let fullText = "";
    for await (const chunk of response) {
...
async function summarizeTips(placeName, tips) {
  try {
    ...
    const config = { temperature: 0.3, top_p: 0.9, max_tokens: 256 };
    const model = "llama3-70b-8192";
...
// server/services/wikipedia-images.ts
var wikipedia_images_exports = {};
__export(wikipedia_images_exports, {
  WikipediaImageService: () => WikipediaImageService,
  wikipediaImageService: () => wikipediaImageService
});
import dotenv3 from "dotenv";
var WikipediaImageService, wikipediaImageService;
var init_wikipedia_images = __esm({
  "server/services/wikipedia-images.ts"() {
    "use strict";
    dotenv3.config();
    WikipediaImageService = class {
       ...
       */
      async findNearestArticle(lat, lon, radius = 1e3) {
        try {
       ...
       */
      async getArticleImage(title, thumbSize = 500) {
        try {
          const url = new URL(this.baseUrl);
          url.searchParams.set("action", "query");
       ...
       * @returns Image data or null
       */
      async getImageByCoordinates(lat, lon, radius = 1e3, thumbSize = 500) {
        try {
       ...
       * @returns Image data or null
       */
      async getImageByPlaceName(placeName, thumbSize = 500) {
        try {
          const searchUrl = new URL(this.baseUrl);
          searchUrl.searchParams.set("action", "query");
          searchUrl.searchParams.set("list", "search");
          searchUrl.searchParams.set("srsearch", placeName);
          ...
          if (searchResults && searchResults.length > 0) {
            ...
            return await this.getArticleImage(title, thumbSize);
          }
          return null;
        } catch (error) {
          console.error("Error getting image by place name:", error);
          return null;
        }
      }
    };
    wikipediaImageService = new WikipediaImageService();
  }
});
...
// server/services/langchain-agent.ts
init_gemini();
init_wikipedia_images();

// server/services/budget-agent.ts
import OpenAI2 from "openai";
import dotenv4 from "dotenv";
dotenv4.config();
var BudgetTrackingAgent = class {
  groqClient;
  model = "llama-3.3-70b-versatile";
  constructor() {
    this.groqClient = new OpenAI2({
      apiKey: process.env.GROQ_API_KEY || "",
      baseURL: "https://api.groq.com/openai/v1"
    });
  }
   ...
   */
  async analyzeSpending(expenses, totalBudget) {
    ...
    const topCategories = Object.entries(categoryBreakdown).map(([category, amount]) => ({
      category,
      amount,
   ...
   */
  async generateInsights(expenses, totalBudget, totalSpent, categoryBreakdown) {
    try {
...
${expenses.slice(-5).map((exp) => `- ${exp.category}: $${exp.amount} - ${exp.description}`).join("\n")}
...
Keep each insight to one sentence.`;
      ...
      const insights = content.split("\n").filter((line) => line.trim().startsWith("-") || line.trim().startsWith("\u2022")).map((line) => line.replace(/^[-•]\s*/, "").trim()).filter((line) => line.length > 0).slice(0, 4);
   ...
   */
  async getRecommendations(expenses, totalBudget, userPreferences2) {
    try {
      const totalSpent = expenses.reduce((sum, exp) => sum + exp.amount, 0);
      const remaining = totalBudget - totalSpent;
      const categoryBreakdown = {};
...
${userPreferences2?.remainingDays ? `Remaining Days: ${userPreferences2.remainingDays}` : ""}
${userPreferences2?.location ? `Location: ${userPreferences2.location}` : ""}
${userPreferences2?.interests ? `Interests: ${userPreferences2.interests.join(", ")}` : ""}
...
Be specific and actionable.`;
      ...
      const content = response.choices[0]?.message?.content || "";
      const tips = content.split("\n").filter((line) => line.toUpperCase().includes("TIP:")).map((line) => line.replace(/TIP:\s*/i, "").trim()).filter((line) => line.length > 0).slice(0, 5);
      const savingOpportunities = content.split("\n").filter((line) => line.toUpperCase().includes("SAVE:")).map((line) => line.replace(/SAVE:\s*/i, "").trim()).filter((line) => line.length > 0).slice(0, 3);
      const categoryAdvice = {};
      content.split("\n").filter((line) => line.toUpperCase().includes("CATEGORY:")).forEach((line) => {
      ...
      return {
        ...
        savingOpportunities: savingOpportunities.length > 0 ? savingOpportunities : [
   ...
   */
  async predictBudgetHealth(expenses, totalBudget, remainingDays) {
    const totalSpent = expenses.reduce((sum, exp) => sum + exp.amount, 0);
    ...
    const variance = spendingValues.reduce((sum, val) => sum + Math.pow(val - avgDaily, 2), 0) / spendingValues.length;
...
var budgetAgent = new BudgetTrackingAgent();

// server/services/photo-agent.ts
import OpenAI3 from "openai";
import dotenv5 from "dotenv";
dotenv5.config();
var PhotoAlbumAgent = class {
  groqClient;
  visionModel = "llama-3.2-90b-vision-preview";
  textModel = "llama-3.3-70b-versatile";
  constructor() {
    this.groqClient = new OpenAI3({
      apiKey: process.env.GROQ_API_KEY || "",
      baseURL: "https://api.groq.com/openai/v1"
    });
  }
   ...
   */
  async generatePhotoCaption(imageData, context) {
    try {
...
Caption:`;
      ...
      if (context?.placeName) {
        return `Amazing time at ${context.placeName}! \u{1F4F8}`;
      } else if (context?.location) {
        return `Exploring ${context.location} \u{1F30D}\u2728`;
      }
      return "A beautiful moment captured \u{1F4F8}\u2728";
    }
  }
  /**
   * Generate captions for multiple photos in batch
   */
  async generateBatchCaptions(photos, style) {
...
DESCRIPTION: description`;
      ...
      const tags = tagsMatch ? tagsMatch[1].split(",").map((t) => t.trim()).filter((t) => t.length > 0).slice(0, 5) : ["travel", "memories"];
...
Make it personal and evocative.`;
      ...
      const title = titleMatch ? titleMatch[1].trim().replace(/^["']|["']$/g, "") : request.tripTitle || "My Travel Album";
      const description = descMatch ? descMatch[1].trim().replace(/\n/g, " ") : `A collection of ${request.photos.length} memorable moments from ${request.tripLocation || "my journey"}.`;
      const highlights = highlightsSection.split("\n").filter((line) => line.trim().match(/^[-•*]/)).map((line) => line.replace(/^[-•*]\s*/, "").trim()).filter((line) => line.length > 0).slice(0, 4);
      const finalHighlights = highlights.length > 0 ? highlights : ["Unforgettable experiences", "Beautiful destinations", "Precious memories"];
      const storyPrompt = `Write a brief, engaging narrative (3-4 sentences) that ties together this travel album:

${photoAnalyses.map((p, i) => `Photo ${i + 1}: ${p.description}`).join("\n")}
...
Create a cohesive story that flows naturally and captures the journey's essence.`;
      ...
      const story = storyResponse.choices[0]?.message?.content?.trim() || `This journey took us through amazing places and unforgettable experiences. Each moment captured here tells a story of adventure, discovery, and joy. From sunrise to sunset, every photo represents a memory we'll treasure forever.`;
      const albumPhotos = photoAnalyses.map((analysis) => ({
        id: analysis.id,
        caption: analysis.caption,
        tags: analysis.tags,
        context: analysis.description,
        alternativeCaptions: []
        // Could generate multiple caption options
      }));
      return {
        title,
        description,
      ...
      return {
        ...
        photos: request.photos.map((photo) => ({
          id: photo.id,
          caption: "A beautiful moment captured \u{1F4F8}",
   ...
   */
  async generateAlternativeCaptions(imageData, originalCaption, count = 3) {
    try {
...
List them one per line.`;
      ...
      const content = response.choices[0]?.message?.content || "";
      const alternatives = content.split("\n").map((line) => line.replace(/^\d+\.\s*/, "").trim()).filter((line) => line.length > 10 && line.length < 200).slice(0, count);
...
var photoAgent = new PhotoAlbumAgent();
...
var TravelPlanningAgent = class {
   ...
   */
  async enrichPlaceWithImage(place) {
    if (place.photoUrl) {
      return place;
    }
    try {
      console.log(`\u{1F5BC}\uFE0F Fetching Wikipedia image for ${place.name}...`);
      let wikiImage = null;
      ...
      if (!wikiImage && place.name) {
        wikiImage = await wikipediaImageService.getImageByPlaceName(place.name, 500);
      }
...
var MultiAgentOrchestrator = class {
   ...
   */
  initializeTravelAgent(context) {
    this.travelAgent = new TravelPlanningAgent(context);
  }
  /**
   * Coordinate all agents to create a comprehensive itinerary
   */
  async createComprehensiveItinerary(context) {
    try {
      console.log("\u{1F3AF} Multi-Agent System: Starting comprehensive itinerary generation...");
      this.initializeTravelAgent(context);
      console.log("\u{1F464} Agent 1 (Discovery): Searching for places...");
      const discoverResult = await this.travelAgent.executeTask("discover_places");
      if (!discoverResult.success) {
        return discoverResult;
      }
      console.log("\u{1F4F8} Agent 2 (Image Enrichment): Adding photos to places...");
      const enrichedPlaces = await Promise.all(
        discoverResult.data.places.map(async (place) => {
          return await this.travelAgent.enrichPlaceWithImage(place);
        })
      );
      context.availablePlaces = enrichedPlaces;
      this.initializeTravelAgent(context);
      console.log("\u{1F5D3}\uFE0F Agent 3 (Planning): Generating optimized itinerary...");
      const itineraryResult = await this.travelAgent.executeTask("generate_itinerary");
      if (!itineraryResult.success) {
        return itineraryResult;
      }
      console.log("\u{1F6E3}\uFE0F Agent 4 (Optimization): Optimizing route...");
      const optimizeResult = await this.travelAgent.executeTask("optimize_route");
      if (!optimizeResult.success) {
        return optimizeResult;
      }
      console.log("\u{1F4B0} Agent 5 (Budget): Analyzing estimated costs...");
      ...
      return {
        ...
        data: {
          ...
          agentContributions: {
            ...
            imageEnrichment: `Enhanced ${enrichedPlaces.filter((p) => p.photoUrl).length} places with photos`,
   ...
   */
  async estimateTripCosts(itinerary, budgetLevel) {
    try {
      ...
      const multiplier = budgetMultipliers[budgetLevel.toLowerCase()] || 1;
      const restaurants = itinerary.filter(
        (p) => p.category?.toLowerCase().includes("restaurant") || p.category?.toLowerCase().includes("food")
      );
      const activities = itinerary.filter(
        (p) => !p.category?.toLowerCase().includes("restaurant") && !p.category?.toLowerCase().includes("food")
      );
      const estimatedFood = restaurants.length * 25 * multiplier;
      const estimatedActivities = activities.length * 15 * multiplier;
      const estimatedTransport = itinerary.length * 5 * multiplier;
      const estimatedTotal = estimatedFood + estimatedActivities + estimatedTransport;
      const breakdown = {
      ...
      const mockExpenses = [
        {
          id: "1",
          category: "Food",
          amount: estimatedFood,
          description: "Estimated dining costs",
          date: (/* @__PURE__ */ new Date()).toISOString()
        },
        {
          id: "2",
          category: "Activities",
          amount: estimatedActivities,
          description: "Estimated activity costs",
          date: (/* @__PURE__ */ new Date()).toISOString()
        },
        {
          id: "3",
          category: "Transport",
          amount: estimatedTransport,
          description: "Estimated transport costs",
          date: (/* @__PURE__ */ new Date()).toISOString()
        }
      ];
   ...
   */
  async analyzeBudget(expenses, totalBudget) {
    return await budgetAgent.analyzeSpending(expenses, totalBudget);
  }
  /**
   * Get budget recommendations
   */
  async getBudgetRecommendations(expenses, totalBudget, preferences) {
   ...
   */
  async generateAlbum(photos, albumInfo) {
...
async function registerRoutes(app2) {
  try {
    ...
    const manifestPath = path3.resolve(import.meta.dirname, "..", "client", "public", "manifest.webmanifest");
    const swPath = path3.resolve(import.meta.dirname, "..", "client", "public", "service-worker.js");
    if (fs2.existsSync(manifestPath)) {
      app2.get("/manifest.webmanifest", (_req, res) => res.sendFile(manifestPath));
    }
    if (fs2.existsSync(swPath)) {
      app2.get("/service-worker.js", (_req, res) => res.sendFile(swPath));
    }
  } catch (e) {
    console.warn("PWA assets not served:", e);
  }
  app2.get("/open", (req, res) => {
    const url = String(req.query.url || "");
    return res.redirect(`/?deep=${encodeURIComponent(url)}`);
  });
  ...
  app2.post("/api/generate-itinerary", async (req, res) => {
    try {
      ...
      if (isMultiDay) {
        orchestrator.initializeTravelAgent(agentContext);
        const discoverResult = await orchestrator["travelAgent"].executeTask("discover_places");
        if (!discoverResult.success) {
        ...
        const enrichedPlaces = await Promise.all(
          validPlaces.map(async (place) => {
            if (!place.photoUrl && place.latitude && place.longitude) {
              const enriched = await orchestrator["travelAgent"].enrichPlaceWithImage(place);
              return enriched;
            }
            return place;
          })
        ...
        const coordinates = [
          ...
          ...enrichedPlaces.map((p) => [p.longitude, p.latitude])
        ];
        ...
        const itineraryForStorage = enrichedPlaces.map((place, index) => {
          const routeInfo = optimizationResult.routes.find((r) => r.from === index);
          return {
            ...place,
            travelTimeToNext: routeInfo?.duration,
      ...
      console.log("\u{1F680} Using Multi-Agent System for comprehensive planning...");
      const result = await orchestrator.createComprehensiveItinerary(agentContext);
      if (!result.success) {
        return res.status(500).json(result);
      }
      const allCoords = [
        [preferences.location.lng, preferences.location.lat],
        ...result.data.itinerary.map((place) => [place.longitude, place.latitude])
      ];
      ...
      const optimizedRoutePath = (result.data.optimizedRoute || []).map((idx) => {
        const c = allCoords[idx];
        return { lat: c[1], lng: c[0] };
      });
      const itinerary = await storage.createItinerary({
        sessionId,
        places: result.data.itinerary,
        totalDuration: Math.round(result.data.totalDuration),
      ...
      res.json({
        success: true,
        itinerary,
        budgetInsights: result.data.budgetInsights,
        agentContributions: result.data.agentContributions,
        routeGeometry,
        // @ts-ignore
        osrmInstructions
      });
  ...
  app2.post("/api/foursquare-places", async (req, res) => {
    try {
      const { latitude, longitude, query, radius, categories, limit } = req.body;
      ...
      const places2 = await foursquareService2.searchPlaces(searchParams);
      res.json(places2);
    } catch (error) {
  ...
  app2.get("/api/foursquare-place/:placeId", async (req, res) => {
    try {
      ...
      const { foursquareService: foursquareService2 } = await Promise.resolve().then(() => (init_foursquare(), foursquare_exports));
      const place = await foursquareService2.getPlaceDetails(placeId);
      res.json(place);
    } catch (error) {
  ...
  app2.post("/api/gemini", async (req, res) => {
    try {
      ...
      res.json({ caption: fullText.trim() });
    } catch (error) {
  ...
  app2.post("/api/budget/analyze", async (req, res) => {
    try {
      const { expenses, totalBudget } = req.body;
      if (!Array.isArray(expenses) || typeof totalBudget !== "number") {
        return res.status(400).json({
          success: false,
          message: "Invalid request: expenses array and totalBudget required"
        });
      }
      const sessionId = req.headers["session-id"];
      const orchestrator = new MultiAgentOrchestrator(sessionId);
      const analysis = await orchestrator.analyzeBudget(expenses, totalBudget);
      res.json({
        success: true,
        analysis
      });
  ...
  app2.post("/api/budget/recommendations", async (req, res) => {
    try {
      const { expenses, totalBudget, preferences } = req.body;
      if (!Array.isArray(expenses) || typeof totalBudget !== "number") {
        return res.status(400).json({
          success: false,
          message: "Invalid request: expenses array and totalBudget required"
      ...
      const sessionId = req.headers["session-id"];
      const orchestrator = new MultiAgentOrchestrator(sessionId);
      const recommendations = await orchestrator.getBudgetRecommendations(
        expenses,
        totalBudget,
        preferences
      );
  ...
  app2.post("/api/budget/predict", async (req, res) => {
    try {
      const { expenses, totalBudget, remainingDays } = req.body;
      if (!Array.isArray(expenses) || typeof totalBudget !== "number" || typeof remainingDays !== "number") {
        return res.status(400).json({
          success: false,
          message: "Invalid request: expenses, totalBudget, and remainingDays required"
        });
      }
      const sessionId = req.headers["session-id"];
      const orchestrator = new MultiAgentOrchestrator(sessionId);
      const prediction = await orchestrator.predictBudgetHealth(
        expenses,
        totalBudget,
        remainingDays
      );
      ...
      res.status(500).json({
        success: false,
        message: error.message || "Failed to predict budget health"
      });
    }
  });
  app2.post("/api/photos/caption", async (req, res) => {
    try {
      ...
      const sessionId = req.headers["session-id"];
      const orchestrator = new MultiAgentOrchestrator(sessionId);
      const caption = await orchestrator.generatePhotoCaption(imageData, context);
      res.json({
        success: true,
        caption
      });
  ...
  app2.post("/api/photos/album", async (req, res) => {
    try {
      const { photos, albumInfo } = req.body;
      ...
      const sessionId = req.headers["session-id"];
      const orchestrator = new MultiAgentOrchestrator(sessionId);
      const album = await orchestrator.generateAlbum(photos, albumInfo);
      res.json({
        success: true,
        album
      });
  ...
  app2.get("/api/wikipedia/image", async (req, res) => {
    try {
      const { lat, lon, name, radius } = req.query;
      const { wikipediaImageService: wikipediaImageService2 } = await Promise.resolve().then(() => (init_wikipedia_images(), wikipedia_images_exports));
      let imageResult = null;
      if (lat && lon) {
        const latitude = parseFloat(lat);
        const longitude = parseFloat(lon);
        const searchRadius = radius ? parseInt(radius) : 1e3;
        if (!isNaN(latitude) && !isNaN(longitude)) {
          imageResult = await wikipediaImageService2.getImageByCoordinates(
            latitude,
            longitude,
            searchRadius
          );
      ...
      if (!imageResult && name) {
        imageResult = await wikipediaImageService2.getImageByPlaceName(name);
      }
      if (imageResult) {
        res.json({
          success: true,
...
var offline = [process.env.VITE_LOCAL_ONLY, process.env.VITE_LOCAL].filter(Boolean).some((v) => ["1", "true", "yes"].includes(String(v).toLowerCase()));
var vite_config_default = defineConfig({
  // Use relative base when offline so built index.html references ./assets/... instead of /assets/...
  base: offline ? "./" : "/",
  plugins: [
    react(),
    runtimeErrorOverlay(),
